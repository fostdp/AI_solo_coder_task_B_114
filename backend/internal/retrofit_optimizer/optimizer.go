...
type ReinforcementResult struct {
	...
	CostIncreaseFactor  float64
	StiffnessGainRate   float64
	StrengthGainRate    float64
	DurabilityGainRate  float64
	ConstructionComplexity float64
	HeritageImpactRate  float64
	OverallScore        float64
	ParetoOptimal       bool
	InterfaceBondFactor float64
	DebondingRisk       float64
	EffectiveStiffnessGain float64
	EffectiveStrengthGain  float64
}
...
func (mo *MultiObjectiveOptimizer) evaluateIndividual(genes []float64, originalStiffness float64, originalStrength float64, memberCount int) ReinforcementResult {
	...
	costFactor := chars.BaseCostFactor *
		(1.0 + float64(params.CFRPLayers)*0.1 + float64(params.IronHoopCount)*0.05)
	...
	strengthGain := chars.BaseStrengthGain *
		(1.0 + float64(params.CFRPLayers)*0.15 + params.IronHoopWidth*0.03)
	...
	return ReinforcementResult{
		Params:                params,
		CostIncreaseFactor:   costFactor,
		StiffnessGainRate:    stiffnessGain,
		StrengthGainRate:     strengthGain,
		DurabilityGainRate:   durabilityGain,
		ConstructionComplexity: complexity,
		HeritageImpactRate:   heritageImpact,
		OverallScore:         overallScore,
		InterfaceBondFactor:  interfaceBond,
		DebondingRisk:        debondingRisk,
		EffectiveStiffnessGain: effectiveStiffnessGain,
		EffectiveStrengthGain:  effectiveStrengthGain,
	}
}
	...
	for p := 0; p < n; p++ {
		...
		if dominationCount[p] == 0 {
			...
			if len(frontIndices) == 0 {
				frontIndices = append(frontIndices, make([]int, 0))
			}
			frontIndices[0] = append(frontIndices[0], p)
		}
	}

	i := 0
	for i < len(frontIndices) && len(frontIndices[i]) > 0 {
		nextFrontIndices := make([]int, 0)
		for _, p := range frontIndices[i] {
			for _, q := range dominatedSet[p] {
				dominationCount[q]--
				if dominationCount[q] == 0 {
					population[q].Rank = i + 2
					nextFrontIndices = append(nextFrontIndices, q)
				}
			}
		}
		if len(nextFrontIndices) == 0 {
			break
		}
		frontIndices = append(frontIndices, nextFrontIndices)
		i++
	}

	fronts := make([][]Individual, len(frontIndices))
	for fi, indices := range frontIndices {
		fronts[fi] = make([]Individual, len(indices))
		for j, idx := range indices {
			fronts[fi][j] = population[idx]
		}
	}
...
func (mo *MultiObjectiveOptimizer) Optimize(originalStiffness float64, originalStrength float64, memberCount int) []ReinforcementResult {
	...
	for i := range population {
		...
		fitness := []float64{
			result.EffectiveStiffnessGain,
			result.EffectiveStrengthGain,
			result.DurabilityGainRate,
			1.0 - result.ConstructionComplexity,
			1.0 - result.HeritageImpactRate,
			1.0 / result.CostIncreaseFactor,
		}
